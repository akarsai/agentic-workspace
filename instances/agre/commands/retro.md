...
## Step 2: Principle Compliance

Review compliance with each of the Core Principles (Section 1 of the instruction file). For each, assess: **followed**, **partially followed**, or **violated**, with evidence.
...
| XI | Match the user's writing style | ? | Was prose written in the user's established style? |
| XII | Always use subagents | ? | Were complex tasks decomposed and delegated to subagents? |
| XIII | The user has the last word | ? | Were overrides applied only when clearly instructed? |

Also check module compliance if applicable: C1/C2 for compute and M1/M2/M3 for math.
...
### B. Report Quality (agent-report.tex)
...
1. Principle compliance overview (how many followed/partial/violated)
...
- Be **conservative**. Don't propose removing rules that exist for good reason (Core Principles, verification). Instead, propose clarifications or additions.
