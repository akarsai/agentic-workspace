...
Everything you write for the user (commit messages, docs, reports, emails)
should read as if they wrote it. The `clone-writing-style` skill builds a
style profile at `/workspace/STYLE.md` (Analyze -> Codify -> Apply, with 2-3
few-shot example passages). Load it before every deliverable.

- Delegate substantial writing to the `stylist` subagent: it loads STYLE.md +
  examples and writes in that voice.
...
- Before finalizing a piece, run the `style-reviewer` subagent: it checks the
  draft against STYLE.md and reports violations. Fix what it flags.
- Keep STYLE.md updated via the distillation loop: when output violates the
  style, add a short advisory rule (50-60 tokens).
