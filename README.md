...
- **`FATAL: Building from a definition file requires root or some kind of fake root`** (Apptainer): the host allows neither fakeroot nor user namespaces for your user. The build is retried once with a private userspace fakeroot (downloaded into `~/.cache/agentic-workspace`, no admin needed), which setuid Apptainer installs run the whole build under — on most HPC login nodes that retry just builds the image. If even that fails (non-setuid Apptainer, no user namespaces), ask an admin to run `sudo apptainer config fakeroot --add $USER` (adds the `/etc/subuid` + `/etc/subgid` entries; needs the `uidmap` package), to enable unprivileged user namespaces, or to install the `fakeroot` package — or build on a host that allows it (e.g. with Docker) and copy the `.sif` files over. `update` keeps previously built images in the meantime (with a warning); explicit `--build` commands fail until the host allows builds.
